...
//------------------------------------------------------------------------------
// ----------------- COVS DIST -------------------------------------------------
// compute the distance among continuous and discrete covariates
...
mat MC_routine(mat Y,
  ...
  double wD = accu(type_of_var == 0) / ((double) type_of_var.n_elem);
  double wC = 1 - wD;
  int mk = Y.n_rows - 1;
  int sample_size;
  ...
  // if there is any continuous covariate, 
  // compute the inverse of the covariance
  mat cov_covs, inv_cov_covs;
  if(use_cont){
    cov_covs = cov(Y.cols(find(type_of_var == 1)));
    inv_cov_covs = inv(cov_covs);
